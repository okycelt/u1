...
public class OGLBuffers implements Cleaner.Cleanable {
    ...
    public OGLBuffers(GL4 gl) {
        this.gl = gl;
    }

    public OGLBuffers(GL4 gl, float[] vertexData, Attrib[] attributes, int[] indexData) {
        this.gl = gl;
    ...
    public OGLBuffers(GL4 gl, float[] vertexData, int floatsPerVertex, Attrib[] attributes, int[] indexData) {
        this.gl = gl;
    ...
    public void addVertexBuffer(float[] data, int floatsPerVertex, Attrib[] attributes) {
        ...
        gl.glBufferData(GL4.GL_ARRAY_BUFFER, data.length * 4, buffer, GL4.GL_STATIC_DRAW);
    ...
    public void setIndexBuffer(int[] data) {
        ...
        gl.glBindBuffer(GL4.GL_ELEMENT_ARRAY_BUFFER, indexBuffer[0]);
        gl.glBufferData(GL4.GL_ELEMENT_ARRAY_BUFFER, data.length * 4, buffer, GL4.GL_STATIC_DRAW);
    }
    ...
    public void bind(int shaderProgram) {
        ...
        for (VertexBuffer vb : vertexBuffers) {
            gl.glBindBuffer(GL4.GL_ARRAY_BUFFER, vb.id);
            int offset = 0;
            for (int j = 0; j < vb.attributes.length; j++) {
                ...
                if (location >= 0) {// due to optimization GLSL on a graphic card
                    ...
                    gl.glVertexAttribPointer(location, vb.attributes[j].dimension, GL4.GL_FLOAT,
    ...
    public void draw(int topology, int shaderProgram) {
        ...
        if (indexBuffer == null) {
            ...
            gl.glDrawElements(topology, indexCount, GL4.GL_UNSIGNED_INT, 0);
        }
        unbind();
    }
    ...
    public void draw(int topology, int shaderProgram, int count, int start) {
        ...
        if (indexBuffer == null) {
            ...
            gl.glDrawElements(topology, count, GL4.GL_UNSIGNED_INT, start * 4);
        }
        unbind();
    }
