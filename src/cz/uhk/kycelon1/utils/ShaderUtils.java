...
public class ShaderUtils {
    ...
    private static final int[] SHADER_NAME_CONSTANTS = { GL4.GL_VERTEX_SHADER, GL4.GL_FRAGMENT_SHADER,
            GL4.GL_GEOMETRY_SHADER, GL4.GL_TESS_CONTROL_SHADER, GL4.GL_TESS_EVALUATION_SHADER, GL4.GL_COMPUTE_SHADER };
     ...
     */
    public static int loadProgram(GL4 gl, String vertexShaderFileName, String fragmentShaderFileName,
     ...
     * @return new id of shader program
     */
    public static int loadProgram(GL4 gl, String shaderFileName) {
        String[] shaderFileNames = new String[SHADER_FILE_EXTENSIONS.length];
     ...
     * @return new id of shader program
     */
    public static int loadProgram(GL4 gl, String[] shaderFileNames) {
     ...
     * @return new id of shader program
     */
    public static int loadProgram(GL4 gl, String[] vertexShaderSrc, String[] fragmentShaderSrc,
     ...
     * @return new id of shader program
     */
    public static int loadProgram(GL4 gl, String[][] shaderSrcArray) {
...
//            }
     ...
     */
    static public int createShaderProgram(GL4 gl, String[] shaderSrc, int type) {
     ...
     * @return new id of shader
     */
    static public int compileShaderProgram(GL4 gl, int shader) {
        String error;

        gl.glCompileShader(shader);
        error = checkLogInfo(gl, shader, GL4.GL_COMPILE_STATUS);
        if (error == null) {
            return shader;
        } else {
     ...
     * @return new id of shader program
     */
    static public boolean linkProgram(GL4 gl, int shaderProgram) {
        String error;
        gl.glLinkProgram(shaderProgram);
        error = checkLogInfo(gl, shaderProgram, GL4.GL_LINK_STATUS);
        if (error == null) {
            return true;
        } else {
            System.err.println("failed");
    ...
    static private String checkLogInfo(GL4 gl, int programObject, int mode) {
        switch (mode) {
            case GL4.GL_COMPILE_STATUS:
                return checkLogInfoShader(gl, programObject, mode);
            case GL4.GL_LINK_STATUS:
            case GL4.GL_VALIDATE_STATUS:
    ...
    static private String checkLogInfoShader(GL4 gl, int programObject, int mode) {
        int[] error = new int[] { -1 };
        gl.glGetShaderiv(programObject, mode, error, 0);
        if (error[0] != GL4.GL_TRUE) {
            int[] len = new int[1];
            gl.glGetShaderiv(programObject, GL4.GL_INFO_LOG_LENGTH, len, 0);
            if (len[0] == 0) {
                return null;
            }
    ...
    static private String checkLogInfoProgram(GL4 gl, int programObject, int mode) {
        int[] error = new int[] { -1 };
        gl.glGetProgramiv(programObject, mode, error, 0);
        if (error[0] != GL4.GL_TRUE) {
            int[] len = new int[1];
            gl.glGetProgramiv(programObject, GL4.GL_INFO_LOG_LENGTH, len, 0);
            if (len[0] == 0) {
                return null;
            }
            byte[] errorMessage = new byte[len[0]];
